...
library(e1071)
library(caTools)
model <- svm(as.matrix(XTrain_scaled), yTrain, scale=F, type= "C-classification",kernel='polynomial')
summary(model)
```


```{r}
pred = predict(model, newdata = as.matrix(XTest_scaled))
#Matrice de confusion
cm = table(pred, yTest)
print(cm)
#Taux d'erreur
...
```{r}
svm2 = svm(as.matrix(XTrain_scaled), yTrain, scale=F, type= "C-classification",kernel='radial',cost = 7)
svm2
```


```{r}
pred2 = predict(svm2, newdata = as.matrix(XTest_scaled))
#Matrice de confusion
cm2 = table(pred2, yTest)
print(cm2)
#Taux d'erreur
err2 = ((cm2[1,2] + cm2[2,1])/sum(cm2))*100
print(err2)
```
...
colAUC(all_pred, yTest, plotROC = FALSE)
#abline(0,1, col = "blue")
```
